...
function ItemsTable({ title, isYou, items }) {
  return (
            ...
            items.map((x, idx) => {
              ...
              return (
                <tr key={idx}>
                  ...
                  <td>
                    {outOfStock ? (
                      <span style={{ color: "#b00020", fontWeight: 700 }}>在庫なし</span>
                    ) : (
                      x.stock_now
                    )}
                  </td>
...
export default function TradeDetail() {
  ...
  const canChat = status === "Accepted" || status === "Shipping" || status === "Completed";

  const myShippedAt = trade
    ? youAreProposer
      ? trade.shipped_proposer_at
      : youAreReceiver
      ? trade.shipped_receiver_at
      : null
    : null;

  const myReceivedAt = trade
    ? youAreProposer
      ? trade.received_proposer_at
      : youAreReceiver
      ? trade.received_receiver_at
      : null
    : null;
  ...
  const view = useMemo(() => {
    ...
    const receiverGives = proposerTake;
    const proposerGives = proposerGive;
    ...
    function enrich(x) {
      ...
      return {
        ...
        trade_qty: Number(x?.quantity ?? 0),
      };
    }

    return {
      left: receiverGives.map(enrich),
      right: proposerGives.map(enrich),
    };
  }, [trade, listings]);
  ...
  async function received() {
    setErr("");
    try {
      await tradeApi.received(tradeId);
      await reload();
    } catch (e) {
      setErr(String(e?.message || e));
    }
    ...
    <div style={{ padding: 16, maxWidth: 1100 }}>
      ...
      {trade ? (
        <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12, marginTop: 10 }}>
          ...
          <div style={{ marginTop: 10 }}>
            <div>発送: 自分 {fmt(myShippedAt)} / 両者発送 {bothShipped ? "済" : "未"}</div>
            <div>到着: 自分 {fmt(myReceivedAt)}</div>
          </div>
          ...
          <div style={{ marginTop: 10 }}>
            ...
            {canDecide ? (
              <>
                ...
                <button onClick={reject} style={{ marginLeft: 6 }}>
                  却下
                </button>
              </>
            ) : null}
            ...
            {canShip ? (
              <button onClick={ship} style={{ marginLeft: 8 }}>
                発送した
              </button>
            ) : null}

            {canReceived ? (
              <button onClick={received} style={{ marginLeft: 8 }}>
                到着した
              </button>
            ) : null}
          </div>
      ...
      {trade ? (
        <div style={{ display: "flex", gap: 12, marginTop: 14, alignItems: "flex-start" }}>
          <ItemsTable title="受け手の出品" isYou={youAreReceiver} items={view.left} />
          <ItemsTable title="提案者の出品" isYou={youAreProposer} items={view.right} />
        </div>
      ) : null}

      <div style={{ marginTop: 16, border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
        <div style={{ fontWeight: 800 }}>チャット</div>

        {!canChat ? (
          <div style={{ marginTop: 8, opacity: 0.8 }}>取引が承諾されると使えます</div>
        ) : (
          <>
